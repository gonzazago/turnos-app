...
export function HeroFunnel({ variant = 'light' }: { variant?: 'light' | 'dark' }) {
  ...
  return (
    <div className="mt-10 flex flex-col items-center gap-6 w-full max-w-lg mx-auto text-left">
      ...
      >
        <div className={`absolute -inset-1 bg-gradient-to-r ${variant === 'light' ? 'from-blue-600 to-indigo-600' : 'from-white to-blue-100'} rounded-2xl blur opacity-25 group-hover:opacity-50 transition duration-1000 group-hover:duration-200`}></div>
        <div className={`relative flex flex-col sm:flex-row items-stretch gap-2 p-2 ${variant === 'light' ? 'bg-white border-slate-200' : 'bg-slate-900 border-slate-700'} rounded-2xl border shadow-xl`}>
          <div className="flex items-center flex-1 min-w-0 px-4 py-3 sm:py-0">
            ...
            <input
              ...
              className={`w-full bg-transparent border-none focus:ring-0 ${variant === 'light' ? 'text-slate-900' : 'text-white'} font-bold placeholder:text-slate-300 placeholder:font-normal`}
              required
            />
          </div>
          <button
